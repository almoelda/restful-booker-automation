...
test.describe('Admin page tests', () => {
  ...
  test('Should successfully login the contact form', async () => {
    ...
    await expect(loginPage.page).toHaveURL('https://automationintesting.online/admin/rooms');
    await expect(loginPage.page.getByRole('link', { name: 'Restful Booker Platform Demo' })).toBeVisible();
    await expect(loginPage.page.locator('[data-testid="roomlisting"]')).toHaveCount(3);
});
